...
| [024](ADR/ADR-024-mining-economy.md) | Mining economy *(economy design session — **proposed**; its six review questions ruled by the owner 2026-08-20, acceptance pending the full read-through)* | **Three ores across 2–3 baked `Site` anchors per system** (ADR-016 §3's reserved kind cashed in), banded by the existing security value with hazards staged pre/post-combat; a fleet `Mine` order with deterministic cycles and a durable site ledger (worlds forget, ledgers do not); holds in integer litres as the first hash-guarded balance content (`Economy.json`, ADR-012 §D13); per-station Bays, manual transfer, and deterministic ME refining with communal station-tier upgrades — Nova-Steel refinable only outside High-Sec |
