...
3. **File names are unique repo-wide** — with a flat layout and project roots on the include
   ...
   | GameLogic | type + family names | `Ids.h` `ShipClass.h` `World.h` `ReplicatedView.h` `Orders.h` `Validate.h` `Formation.h` `Eta.h` `WorldHash.h` `Snapshot.h` `OrderMessages.h` `SchemaHash.h` `Universe.h` `UniverseParse.h` `UniverseGen.h` `WorldRegistry.h` `Transfer.h` `Station.h` `StationMessages.h` `FleetSummary.h` `EventRecord.h` |
