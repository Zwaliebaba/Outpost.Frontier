...
3. **File names are unique repo-wide** — with a flat layout and project roots on the include
   ...
   | Outpost | — | `Main.cpp` `AppConfig.h` `AppConfig.cpp` `ConfigLoad.h` `ConfigLoad.cpp` `UniverseLoad.h` `UniverseLoad.cpp` `ParkedFleetView.h` `ParkedFleetView.cpp` `SelfTest.h` `SelfTest.cpp` |
