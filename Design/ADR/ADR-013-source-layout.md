...
3. **File names are unique repo-wide** — with a flat layout and project roots on the include
   ...
   | NeuronServer | type names | `NeuronServer.h` `Simulation.h` `ServerHost.h` `ServerConfig.h` `SnapshotSender.h` — plus one **reserved name, not yet a file**: `Session.h` (per-connection state, still a struct inside `ServerHost.cpp`). `SnapshotSender.h` stopped being reserved at T2: it is the per-client path ADR-018 A13 and [ADR-022](ADR-022-interest-and-delta.md) §1 both require, and one session owns one of them |
