...
3. **User layer for settings:** `Settings.json` under
   ...
   **only** file the game writes. It carries exactly the keys the settings screen owns
   ...
   >
   > > **N3 built it, 2026-08-23 — for three families of the four.** The settings screen writes
   > > back through `ClientApp::Config`, which the composition root maps into `AppConfig` at
   > > shutdown exactly as `MakeClientConfig` maps the other way. **Accessibility** (the UI scale,
   > > the palette, three readability rules) and a new **input** family (handedness, the
   > > long-press dwell) are new keys here; **display** (vsync, frame cap) writes back through the
   > > keys that already existed.
   > >
   > > **Audio does not**, and the reason is a client API rather than this layer: `AudioDevice`
   > > takes its gains at creation and has no setter for a running mixer, so a volume slider would
   > > only take effect at next start — which the screen's own CHANGES APPLY IMMEDIATELY promise
   > > forbids. The section is drawn and refused with that sentence on it. **And the F1
   > > diagnostics strip still has no path** to tell the composition root it was pressed, which is
   > > the other half of what this note named and is untouched.
   > >
   > > One thing N3 had to decide that this ADR implies but does not state: the layer is written
   > > **only when the player actually changed something** (`ClientApp::SettingsChanged`). §A3 says
   > > the file records *changes and not state*, so a session that opened nothing should leave it
   > > byte-for-byte as it found it, timestamp included.
   >
