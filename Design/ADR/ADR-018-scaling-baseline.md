...
> **Built at U3c, 2026-08-21, and "fixed to one value" is no longer true.** `ServerHost` mints
> a `PlayerId` per **player**, starting at `SOLE_PLAYER_ID` so the first client is still player
> one; sessions lapse rather than end and resume inside `SESSION_GRACE_SECONDS`, in
> `NeuronServer/SessionResume.h`. `Hello`/`Welcome` needed **no schema bump** — the id and the
> token have been on the wire since T2 shipping as zero, which is exactly the bump this
> paragraph spent in advance.
>
> Two things this decision got right and one it did not say. **Right:** keying on the player
> rather than the connection made a resumed session pick its whole world back up by carrying
> one integer, and made ownership a filter rather than a restructure. **Right:** minting the
> distinction before accounts existed meant the accessors already took the id — every one of
> them was a one-line change when the filter became real. **Not said:** *the simulation had no
> notion of ownership at all.* Every accessor took the `PlayerId` and ignored it, so U3c had to
> split, and half of it was building `ShipId → PlayerId` at the universe layer before the
> second id could prove anything. "Everything player-keyed keys on `PlayerId`" was true of the
> *signatures* and of nothing else.
>
> **The resume token is a handle and not a credential**, and `SessionResume.h` says so at
> length. Nobody is authenticated; whoever holds one is treated as the player. It is OS-seeded
> so it cannot be counted to and rotated per use so one seen on the wire is worth one
> reconnect. This ADR declined to mint a token at T2 because inventing one would be inventing a
...
| A5 | ~~Anchor record carries the D18 offset rule and D6a's derived occupant ids from the first bake; warp-in invariant restated against the D7 radius~~ **Done 2026-08-19 with U1:** `Anchor` carries `warpInPoint`, `warpInFacingTurns16`, `arrivalSpreadRadiusCm`, `undockPoint`, `undockFacingTurns16` and the `occupantIdBase`/`occupantCount` pair, all baked and round-tripped. The offset *rule* is U3a's — what U1 owed was that contention never forces an anchor-schema migration, and the field it needs is in the file. Occupant ids are derived from the anchor rather than counted at spawn, so a recreated grid reproduces them exactly; blocks go only to anchors that author something, which is what keeps the highest authored id (26,848) inside the u16 window D6 holds. **Re-measured 2026-08-20 (U4):** gates began authoring their entity, and the block shrank from 8 to 2 to pay for it — 3,356 station anchors and 6,000 gate anchors at eight ids each would have been 74,848, past the u16 ceiling entirely, let alone the 32,767 below `DYNAMIC_SHIP_ID_BASE`. At two the committed universe uses 18,712 (57 %) and the worst case a recipe at this scale can ask for is ~30,000, and the bake now *refuses* rather than wrapping if one ever exceeds it. The room a block bought was room to add an occupant without moving anybody else's id, which is worth something for a save file and nothing for a file that is re-baked from its recipe | **U1** | D18, D6a, D7 |
...
| A11 | ~~u32 ship ids in ... rosters, `StationCommand`~~ **Done 2026-08-22 with U3d-b** (the remainder below landed with the delta cluster, as staged). *Partly done 2026-08-19:* `StationCommand` and `StationRoster` carry u32 ship ids on the wire, and an id past this build's u16 range narrows to `INVALID_SHIP_ID` rather than truncating -- a truncating cast would validate a command against the wrong hull. The *sim's* `ShipId` stays u16, which is D6's own staging: `EntityRecord` cannot widen until the delta cluster removes the full-fit constraint | **T1/T2 clusters** | D6 (SIM-1) |
| A12 | ~~`PlayerId` + reserved token/resume in `Hello`/`Welcome`~~ **Done 2026-08-19 with T2's wire half:** both messages carry a `PlayerId` and a reserved `resumeToken`, `PROTOCOL_VERSION` went to 2 because the schema hash cannot gate the message that carries the schema hash, and the server answers `SOLE_PLAYER_ID` -- a *value*, not the connection id, which is the entire point of minting the distinction before accounts exist. ~~*Still owed:* sessions actually surviving a disconnect for the grace window, and the player-keyed state that keys on it~~ **Both done 2026-08-21 with U3c:** `ResumeTable` files a lapsed session and claims it back inside the window, and the player-keyed state is real rather than nominal -- `WorldRegistry` keeps a `ShipId` -> `PlayerId` index and the accessors filter on it. The token stopped being reserved and started being minted, with no layout change, which is what this row bought | **T2 cluster** | D5 (NET-2) |
| A13 | ~~`SnapshotSender` built per-client~~ **Done 2026-08-20, and the shape paid for itself on 2026-08-22:** U3d-b turned this object into the culling authority ADR-022 §1 named — the ranked list, the byte budget, the ring of views as sent, the delta encoder and the keyframe path all live here, and not one call site moved to put them there, which is what building it per-client from the first line bought. *As built in T2:* `NeuronServer/SnapshotSender.h/.cpp` took its reserved name. One feed per session, owned by the session and constructed with it, and `Simulation::WriteSnapshot` grew a **`PlayerId` viewer** — ignored by today's implementation, because one grid and one commander means every viewer is owed the same bytes, and present in the *seam* because that is what U3b's per-grid views and ADR-022's culling both arrive through. `ServerHost::BroadcastSnapshot` is gone. ~~over-cap refusal tested loudly~~ **Done the same day:** the refusal is counted and logged **per client** (a global counter cannot answer *which* client is over cap), and `AGridPastTheDatagramCapRefusesLoudlyAndRecovers` pins that nothing reaches the wire, that the tick is counted, and that the feed resumes without the session being rebuilt. ~~`StationRoster` addressed per viewer~~ **Done 2026-08-20:** the family got its frame. **One engine wire type, `Summary`, carries every member** (owner's call) — a roster, a fleet summary and whatever ADR-016 §6 adds next are all "what this viewer is owed at ~1 Hz", and which one a payload holds is a byte in `GameLogic/SummaryMessages.h` under the game's own hash rather than an enumerator spent in NeuronCore. No length prefixes: each body already begins with its own row count, so an unknown kind is **refused, not skipped** — skipping would hide the schema disagreement the handshake exists to catch. `Simulation::WriteSummaries` is the seam, the sender owns the cadence (staggered by viewer id, so a shard does not serialise every session on one tick), and the composition root answers it by asking the registry where *this* commander's ships are — every `Docked` row naming a station whose roster is worth sending, so the frame cannot disagree with itself. Summaries go in first and rosters fill what is left; what does not fit is dropped and **counted**, because paging the family is ADR-016 §6's own problem | **T2 / U3b** | D3, D4 (NET-1, SIM-4) |
...
| A25 | ~~U3c — the second-commander gate: two clients, distinct `PlayerId`s, disjoint grids, full loop each over real loopback~~ **Done 2026-08-21 (run 188).** Split into U3c-a (ownership in the simulation) and U3c-b (the wire), because a ship had no owner and a second id against that registry would have passed every privacy assertion for the wrong reason. *What the gate actually found, which is the row's value:* four assumptions that were true while there was one commander and silently wrong once there were two — the start grid was everyone's grid, the shard's `WorldMeta` was everyone's description, `ServedWorld()` was everyone's world so **any commander could order any hull on it**, and the self test's oldest order fixture named `ships.front()`, which on a station grid is the station. None of them would have arrived as a bug report. `NotOwned` is returned for the first time since the MVP reserved it | **New slice after U3b** | D3 (NET-1/2) |
