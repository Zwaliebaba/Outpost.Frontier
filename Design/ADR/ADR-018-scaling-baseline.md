...
| A8 | World-isolation invariant in the registry API; permuted-world-tick-order bit-identity test; build the ADR-007 §7 owner-assert | **U2** | D1a (CPP-1, SIM-6) |
| A9 | Transfer bus lands with `transferId` total order; transits + bus fold into the registry hash | **T1** | D17 (SIM-3) |
| A10 | Dock validation uses the footprint-derived radius; ADR-017 §5 arithmetic corrected; protection window re-checked against the class table | **T1** | D7 (UX-1) |
| A11 | u32 ship ids in sim, registry, logs, rosters, `StationCommand`/`OrderSubmit` | **T1/T2 clusters** | D6 (SIM-1) |
| A12 | `PlayerId` + reserved token/resume in `Hello`/`Welcome`; sessions survive disconnect (grace window); all new player-keyed state keys on it | **T2 cluster** | D5 (NET-2) |
...
| A14 | Interest/delta ADR drafted (scope fixed in D4, includes `EntityRecord` → u32 id and the ownership field); implementation slice scheduled after U3c, gating shared grids | **Draft during T-phase; slice after U3c** | D4, D6 (NET-1, SIM-4, SIM-7, UX-2) |
...
| A17 | Event record producer at the universe layer; U/T slices emit from first events | **T1/U3a onward** | D19 (UX-4) |
