...
inline constexpr std::uint64_t FnvOffsetBasis64 = 0xcbf29ce484222325ull;
inline constexpr std::uint64_t FnvPrime64 = 0x00000100000001b3ull;

/// Hashes bytes, continuing from an existing value so a hash can be accumulated.
[[nodiscard]] constexpr std::uint64_t HashBytes(const void* _data, std::size_t _byteCount, std::uint64_t _seed = FnvOffsetBasis64) noexcept
{
...
[[nodiscard]] constexpr std::uint64_t HashText(std::string_view _text, std::uint64_t _seed = FnvOffsetBasis64) noexcept
{
  std::uint64_t hash = _seed;
...
template <typename T>
[[nodiscard]] std::uint64_t HashValue(const T& _value, std::uint64_t _seed = FnvOffsetBasis64) noexcept
{
