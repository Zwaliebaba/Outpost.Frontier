// Nebula.hlsl -- the ambient field the fleet sits inside (ADR-006 §1).
//
...
// FrameConstants (b0) is deliberately not declared here. This pass reads nothing
// from it, and declaring a cbuffer it does not use would mean a second copy of
// that layout -- including the two array sizes -- free to drift from
// Opaque.hlsl's. The root signature is a superset; an unused slot is not an
// error, and NebulaPass::Record does not bind one.
//
// Must match PassConstants in GpuPipelines.h and the declaration in
// Opaque.hlsl. Two shaders declaring one cbuffer differently is a class of bug
// with no error message, so the three are edited together or not at all.
