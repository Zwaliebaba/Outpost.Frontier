 ...
 * list for the order puck and ghost polylines. This file is (A). (B) arrives
 * with orders in S9.
 *
...
struct OverlayMark
{
  ...
  float radiusMetres = 0.0f;          // Rings: the circle's radius on the plane.
...
struct OverlayTuning
{
  ...
  std::uint32_t ringColourRgba = 0xff64c8ffu;   // Selection: the sheet's cyan.
  std::uint32_t staleRingColourRgba = 0xff4080ffu; // A frozen ship's ring, dimmer and warmer.
  std::uint32_t hullColourRgba = 0xff50e050u;   // Green.
  std::uint32_t shieldColourRgba = 0xffe0c040u; // Amber.
};
